...
function toast(ach){{if(!ach)return;const d=document.createElement('div');d.setAttribute('role','status');d.setAttribute('aria-live','polite');d.style.cssText='position:fixed;top:1rem;right:1rem;background:var(--warn);color:#000;padding:1rem;border-radius:8px;z-index:9999;font-weight:600;animation:fadeIn 0.3s';d.textContent=ach.icon+' '+ach.name+'!';document.body.appendChild(d);setTimeout(()=>d.remove(),3000)}}

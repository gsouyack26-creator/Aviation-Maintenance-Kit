...
EXT17_MODULES = [
    {
      ...
      {"heading": "Jacking Safety Sequence", "body": "Before jacking, confirm the aircraft is on a level, load-rated surface, chocks removed only as required by the AMM procedure, and all doors/panels that could shift CG are closed. Jack at the manufacturer's designated jack points only - never on skin, control surfaces, or unmarked structure. Raise jacks slowly and evenly, watching bubble levels, and install safety locks/pins as soon as the target height is reached; never rely on hydraulic pressure alone to hold a jacked aircraft. Keep the work area clear of unnecessary personnel and never work under a jacked aircraft that is not additionally supported by stands."},
        ],
        "quiz": [
      {"q": "Once an aircraft is raised to the desired height on jacks, the mechanic should:", "choices": ["Rely on hydraulic pressure to hold the height", "Immediately remove the jacks", "Engage the jack's mechanical safety locks/pins before working", "Leave the area unattended"], "answer": 2, "explain": "Mechanical safety locks or pins must be engaged so the aircraft's weight is never held by hydraulic pressure alone."},
    ...
    },
    {
      ...
      {"heading": "Systematic Avionics Fault Isolation", "body": "Effective avionics troubleshooting follows a logical sequence: verify the complaint is reproducible, check power/ground/bus voltage at the unit first (many 'bad' LRUs are actually starved for clean power), then check wiring/connector integrity (pin corrosion, bent pins, poor mating) before suspecting the box itself. Built-in test (BIT) codes and maintenance logs from the unit's non-volatile memory should be reviewed, since intermittent faults often leave a fault-code trail even when the symptom isn't present during the current troubleshooting session."},
        ],
        "quiz": [
      {"q": "Before suspecting a faulty avionics LRU (line replaceable unit), a mechanic should first check:", "choices": ["Nothing, replace the unit immediately", "Power, ground, bus voltage, and wiring/connector integrity", "Only the software version", "The paint on the exterior"], "answer": 1, "explain": "Many apparent LRU failures are actually caused by power/ground issues or connector problems upstream of the unit itself."},
    ...
    },
    {
      ...
      {"heading": "High-Pressure Oxygen Cylinder Handling", "body": "High-pressure oxygen cylinders (often 1800-2000 psi when full) must be handled as potential projectiles if the valve is ever sheared off - always use a valve cap during transport/storage and secure cylinders upright with straps. Before connecting a servicing cart, verify the cylinder's hydrostatic test date is current and that the cylinder is oxygen-service rated (not a generic compressed-gas cylinder). Leak-check all connections with an oxygen-compatible leak detector (never a soap solution containing oil) after servicing."},
        ],
        "quiz": [
      {"q": "Why must a valve cap always be installed on a stored/transported high-pressure oxygen cylinder?", "choices": ["To keep it clean only", "A sheared valve on an uncapped pressurized cylinder can turn it into a dangerous projectile", "It is optional", "To prevent rust"], "answer": 1, "explain": "If the valve of a fully pressurized cylinder is struck and sheared off without a protective cap, the cylinder can become an uncontrolled projectile."},
    ...
    },
    {
      ...
      {"heading": "Performance Trend Monitoring", "body": "Engine condition trend monitoring (ECTM) tracks parameters like EGT margin, fuel flow, N1/N2, and oil consumption over many flights to catch gradual deterioration before it becomes a failure. A sudden EGT margin drop across one or two flights (versus a slow gradual decline over months) points to a different problem - likely FOD, a seal failure, or a nozzle guide vane issue - and warrants immediate investigation rather than routine trending. Ambient conditions (temperature, altitude, bleed/AC loads) must be corrected out of the raw data before comparing trend points, since a hot day alone can look like performance loss if not corrected."},
        ],
        "quiz": [
      {"q": "A sudden EGT margin drop over just one or two flights, unlike a slow gradual decline, most likely indicates:", "choices": ["Normal ambient temperature variation", "A distinct new problem such as FOD, seal failure, or nozzle guide vane damage requiring immediate investigation", "Nothing of concern", "Only a sensor calibration drift"], "answer": 1, "explain": "A sharp sudden EGT margin drop points to an acute new fault rather than gradual wear, and should be investigated immediately."},
    ...
    },
    {
      ...
      {"heading": "Finish Application Troubleshooting", "body": "Common paint defects and their causes: <b>orange peel</b> (texture like citrus skin) from spray gun too far away, too little reducer, or improper air pressure; <b>fisheyes</b> (small craters) from silicone/oil contamination on the surface; <b>runs/sags</b> from too much material applied at once or spraying too close. Always verify surface prep (clean, scuff/etch, correct primer) before topcoat - poor adhesion later shows up as peeling or cracking, often only after months of temperature cycling and UV exposure."},
        ],
        "quiz": [
      {"q": "Fisheyes (small craters) in a paint finish are most often caused by:", "choices": ["Too much reducer", "Silicone or oil contamination on the surface before painting", "Correct air pressure", "Using the right primer"], "answer": 1, "explain": "Fisheyes form when silicone, wax, or oil contamination on the surface repels the wet paint, leaving small craters."},
